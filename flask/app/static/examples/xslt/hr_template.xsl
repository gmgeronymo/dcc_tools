...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template match="/">
    <html>
	  ...
	  .a4-content {
	  padding: 15mm;
	  height: calc(297mm - 30mm); /* Account for padding */
	...
	<div class="a4-page">
	  ...
	  <div class="a4-page">
		...
		<xsl:for-each select="//dcc:usedMethods/dcc:usedMethod[dcc:name/dcc:content != 'Declaração da Incerteza de Medição']">
  ...
  <!-- Template for page header (used on pages 2, 3, 4, etc.) -->
    ...
    <div class="page-header">
      ...
      <table class="tabela-header">
        <tr>
          ...
          <td style="border: 0px; width: 60px;"> </td>
        </tr>
      </table>
      <hr style="border-width: 2px;"/>
...
<xsl:template name="get-token-at-position">
  ...
  <xsl:template name="format-number-comma">
    ...
    <xsl:choose>
      ...
      <!-- Handle integers -->
      <xsl:otherwise>
        <xsl:value-of select="$normalizedNumber"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
...
<xsl:template name="format-decimal-comma">
  ...
  <xsl:template name="format-unit">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$unit = '\micro\volt\volt\tothe{-1}'">&#xB5;V/V</xsl:when>
      <xsl:otherwise>
